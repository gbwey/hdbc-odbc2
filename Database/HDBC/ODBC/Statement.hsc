...
fakeExecute' :: SState -> IO ([SqlColDesc], [SqlColDesc])
...
wrapStmt sstate = Statement
  ...
  , getColumnNames = readMVar (colinfomv sstate) >>= (return . map colName)
...
fdescribetable :: DbcWrapper -> String -> IO [SqlColDesc]
...
fexecute :: SState -> [SqlValue] -> IO (Either Int [SqlColDesc])
...
fnextResultSet :: SState -> IO (Maybe (Either Int [SqlColDesc]))
...
mkBindCol sstate cstmt col = do
  ...
  let colDesc = colInfo !! ((fromIntegral col) -1)
...
fgetcolinfo :: SQLHSTMT -> IO [SqlColDesc]
fgetcolinfo cstmt =
...
fgetparminfo cstmt =
    ...
    where getname icol = -- alloca $ \colnamelp ->
              ...
              do poke datatypeptr 127 -- to test if sqlDescribeParam actually writes something to the area
                 ...
                 colsize  <- peek colsizeptr
                 nullable <- peek nullableptr
                 return $ fromOTypeInfo "" datatype colsize nullable
